...
#if LEVEL_CHARGING
static int level_charging_enable;
...
#if LEVEL_CHARGING
	if (USB_LEVEL_CHARGING_CNT == usb_continuous_notify_times)
		level_charging_enable = 1;
	else
		level_charging_enable = 0;
#endif
}
/**********************************************************
...
#if LEVEL_CHARGING
    if (nonstand_detect_times >= NOSTAND_LEVEL_CHARGING_CNT
...
/**********************************************************
*  Function:       charge_turn_on_force_ico
*  Description:    charge start level charging by forcing ico
*  Parameters:   di:charge_device_info
*  return value:  NULL
**********************************************************/
static int charge_turn_on_force_ico(struct charge_device_info *di, unsigned int *pIdpm_limit_val)
{
    int ret = 0;
    unsigned int vterm = 0;
    di->charge_enable &= di->sysfs_data.charge_enable;

    if (!di->ops->force_ico ||!di->charge_enable)
        return -1;
    ...
    /*enable force ico*/
    ret = di->ops->force_ico(pIdpm_limit_val);
    if (ret) {
        hwlog_err("set force ico fail!!\n");
        ret = -1;
    }
    hwlog_info
        ("input_current is [%d],charge_current is [%d], terminal_voltage is [%d],charge_enable is [%d],idpm_limit_val is [%d]\n",
        di->input_current,di->charge_current,vterm,di->charge_enable, *pIdpm_limit_val);

    return ret;
}

/**********************************************************
*  Function:       select_level_charging_current
*  Description:    level charging when charger type is CHARGER_TYPE_2_USB
                        or CHARGER_TYPE_NON_STANDARD over 3 times
*  Parameters:   di:charge_device_info
*  return value:  NULL
**********************************************************/
static void select_level_charging_current(struct charge_device_info *di)
{
    int idpm_limit_val = 0;
    static int iin_current_final = 0;
    static int ichrg_current_final = 0;

    if((di->sysfs_data.support_level_charging == 1)
             && (1 == level_charging_enable)) {
        if(di->pre_start_attemp_level_charging) {
            di->pre_start_attemp_level_charging = 0;
            di->start_attemp_level_charging = 1;
        }
        if(di->start_attemp_level_charging) {
            iin_current_final = di->input_current;
            ichrg_current_final = di->charge_current;
            hwlog_info
                ("start_attemp_level_charging: iin_current_final is %i, ichrg_current_final is %i\n",
                iin_current_final, ichrg_current_final);
                di->input_current = di->core_data->iin_max;
                di->charge_current = di->core_data->ichg_max;
                hwlog_info
                     ("start_attemp_level_charging: di->input_current is %i, di->charge_current is %i\n",
                     di->input_current, di->charge_current);
                if(!charge_turn_on_force_ico(di, &idpm_limit_val)) {
                    iin_current_final = idpm_limit_val > iin_current_final ? idpm_limit_val : iin_current_final;
                    ichrg_current_final = idpm_limit_val > ichrg_current_final ? idpm_limit_val : ichrg_current_final;
                    di->start_attemp_level_charging = 0;
                    hwlog_info
                        ("charge_turn_on_force_ico success, iin_current_final is %i, ichrg_current_final is %i, idpm_limit_val is %i\n",
                        iin_current_final, ichrg_current_final, idpm_limit_val);
               }
        }
        hwlog_info
            ("charge_select_charging_current: iin_current_final is %i, ichrg_current_final is %i\n",
            iin_current_final, ichrg_current_final);
...
#ifdef LEVEL_CHARGING
    if(di->support_level_charging)
        select_level_charging_current(di);
#endif
...
static void charge_start_charging(struct charge_device_info *di)
{
    ...
    di->check_full_count = 0;
    di->pre_start_attemp_level_charging = 1;
    /*chip init*/
...
static struct charge_sysfs_field_info charge_sysfs_field_tbl[] = {
    ...
    CHARGE_SYSFS_FIELD_RW(support_level_charging,    SUPPORT_LEVEL_CHARGING),
};
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case CHARGE_SYSFS_INPUTCURRENT:
        ...
        break;
    case CHARGE_SYSFS_SUPPORT_LEVEL_CHARGING:
        return snprintf(buf,PAGE_SIZE, "%d\n", di->sysfs_data.support_level_charging);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    case CHARGE_SYSFS_ICHG_THERMAL:
        ...
        else
        {
            ...
            {
                hwlog_info("THERMAL set charge current = %u is less than 500mA, main = %u, aux = %u\n", \
                                  di->sysfs_data.ichg_thl, di->sysfs_data.ichg_thl_main, di->sysfs_data.ichg_thl_aux);
                di->sysfs_data.ichg_thl = 500;
            }
    ...
    case CHARGE_SYSFS_VR_CHARGER_TYPE:
     ...
     case CHARGE_SYSFS_SUPPORT_LEVEL_CHARGING:
         if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > 1))
              return -EINVAL;
         di->sysfs_data.support_level_charging = val;
              hwlog_info("SUPPORT_LEVEL_CHARGING = %d\n", di->sysfs_data.support_level_charging);
        break;
    default:
        hwlog_err("(%s)NODE ERR!!HAVE NO THIS NODE:(%d)\n",__func__,info->name);
        break;
...
static int charge_probe(struct platform_device *pdev)
{
    ...
    di->sysfs_data.support_level_charging = 1;
    ...
    ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
        "support_level_charging", &di->support_level_charging);
    if (ret)
        hwlog_err("get support_level_charging fail!\n");
