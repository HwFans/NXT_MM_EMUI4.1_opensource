...
#else
	i=0;	//not used, because define PGM_BURST_WR
		...
		if(ret != 0){
			...
			for(j=0; j<NVM_PAGE_SIZE; j++){
				if(ucode[addr+j] != read[j])
					TS_LOG_ERR("index:%d %02x : %02x\n", j, ucode[addr+j], read[j]);
			}
			...
			if (!dsm_client_ocuppy(tp_dclient)) {
				TS_LOG_INFO("hideep ic fw update fail, record in dmd\n");
				dsm_client_record(tp_dclient, "TP firmware update fail detected\n");
