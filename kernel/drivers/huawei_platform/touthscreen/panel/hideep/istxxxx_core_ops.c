...
/*
 * if detect one 10 release event and the interval between ap reset to 10 release event received
 * is larger then 500ms, we find the ic crash reset, if the interval time is smaller then 500ms, we
 * treated this 10 release event as normal resume event.
*/
#define  FINGER_TEN_RELEASE_ONE	1
/*
 * two 10 release event, the first is the normal 10 release event which is caused by ap resume reset.
 * and the other is ic abnormal reset when it check the baseline is wrong.
*/
#define  FINGER_TEN_RELEASE_TWO	2

struct ten_finger_oper{
	int ten_finger_press_record;
	int ten_finger_release_record;
	struct timeval reset_tv;
};
struct ten_finger_oper g_ten_finger_oper;
...
s32 hideep_get_image(struct ist510e *ts)
{
	...
	if (NULL == debug_dev->im_buff){
		TS_LOG_ERR("%s input buffer is null\n", __func__);
		return -EINVAL;
	}
...
void hideep_reset_ic(void)
{
	...
	g_ten_finger_oper.ten_finger_press_record = 0;
	g_ten_finger_oper.ten_finger_release_record = 0;
	do_gettimeofday(&(g_ten_finger_oper.reset_tv));
...
static int hideep_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if(release_count>=TOUCH_MAX_COUNT){
		g_ten_finger_oper.ten_finger_release_record++;
		ten_release_finger_tag = 1;
	}
	if (down_count>=TOUCH_MAX_COUNT)
		g_ten_finger_oper.ten_finger_press_record++;

	if (ten_release_finger_tag){
		rel_dwn_diff = g_ten_finger_oper.ten_finger_release_record
						- g_ten_finger_oper.ten_finger_press_record;
		switch(rel_dwn_diff){
			case FINGER_TEN_RELEASE_ONE:
				do_gettimeofday(&tv);
				delta_time = (tv.tv_sec - g_ten_finger_oper.reset_tv.tv_sec)*1000000
							+ tv.tv_usec - g_ten_finger_oper.reset_tv.tv_usec;
				TS_LOG_INFO("10 release after system resume, "
					"ten_finger_release:%d, ten_finger_press:%d, delta_time:%lu\n", 
					g_ten_finger_oper.ten_finger_release_record,
					g_ten_finger_oper.ten_finger_press_record, delta_time);
				if (delta_time > 500000){
					TS_LOG_ERR("ic reset for fw crash, set glove holster roi mode again, "
						"ten_finger_release:%d, ten_finger_press:%d, delta_time:%lu\n", 
						g_ten_finger_oper.ten_finger_release_record,
						g_ten_finger_oper.ten_finger_press_record, delta_time);

					g_ten_finger_oper.ten_finger_release_record = 1;
					g_ten_finger_oper.ten_finger_press_record = 0;
					hideep_item_switch(TS_ACTION_WRITE, VR_GLOVE, 1, &ts->glove_mode);
					hideep_holster_switch(&g_ts_data.feature_info.holster_info);
					hideep_roi_switch(&g_ts_data.feature_info.roi_info);

#if defined (CONFIG_HUAWEI_DSM)
					if (!dsm_client_ocuppy(tp_dclient)) {
						TS_LOG_INFO("ic reset for fw crash, record in dmd\n");
						dsm_client_record(tp_dclient, "TP firmware crash detected\n");
						dsm_client_notify(tp_dclient, DSM_TP_FW_CRASH_ERROR_NO);
					}
#endif
				}
				break;
			case FINGER_TEN_RELEASE_TWO:
				TS_LOG_ERR("ic reset for baseline error, set glove holster roi mode again, "
					"ten_finger_release:%d, ten_finger_press:%d\n", 
					g_ten_finger_oper.ten_finger_release_record,
					g_ten_finger_oper.ten_finger_press_record);
				...
				break;
			default :
				TS_LOG_ERR("unknown ten_finger_release:%d, ten_finger_press:%d\n", 
					g_ten_finger_oper.ten_finger_release_record,
					g_ten_finger_oper.ten_finger_press_record);
				break;
		}
...
static int hideep_holster_switch(struct ts_holster_info *info)
{
	...
	if(!info){
		TS_LOG_ERR("hideep_holster_switch info is NULL\n");
		ret = -ENOMEM;
		return ret;
	}
