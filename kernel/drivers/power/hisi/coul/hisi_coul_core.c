...
#define BASP_FCC_LERAN_TEMP_MIN (100) /*tenth degree*/
...
void basp_record_fcc(struct smartstar_coul_device *di)
{
    ...
    pinfo->real_fcc_record[index - 1] = di->fcc_real_mah;
    for(i = 0;i < MAX_RECORDS_CNT;i++)
    {
...
static void coul_core_get_dts(struct smartstar_coul_device *di)
{
    ...
    {
        int i = 0;
        u32 one_policy[POLICY_MEM_CNT];
        for (i = 0; i < BASP_LEVEL_CNT; i++) {
            if (of_property_read_u32_planar_array(np, "basp_policy", one_policy, POLICY_MEM_CNT, i)) {
                hwlog_err(BASP_TAG"dts:get basp_policy[%d] fail!\n", i);
                break;
            }
            if (one_policy[POLICY_MEM_LEVEL] != (BASP_LEVEL_CNT - 1 - i)) {
                hwlog_err(BASP_TAG"dts:basp_policy[%d] level wrong:[%d]!\n", i, one_policy[POLICY_MEM_LEVEL]);
                break;
            }
            basp_policy[i].level = one_policy[POLICY_MEM_LEVEL];
            basp_policy[i].chg_cycles = one_policy[POLICY_MEM_CHG_CYCLES];
            basp_policy[i].fcc_ratio = one_policy[POLICY_MEM_FCC_RATIO];
            basp_policy[i].volt_dec = one_policy[POLICY_MEM_VOLT_DEC];
            basp_policy[i].cur_ratio = one_policy[POLICY_MEM_CUR_RATIO];
