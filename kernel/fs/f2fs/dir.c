				...
				unsigned int flags)
{
	...
	if (de) {
		*res_page = dentry_page;
		if (flags == CASE_INSENSITIVE && dentry_blk_addr != NULL) {
			*dentry_blk_addr = (unsigned long)dentry_blk;
		}
	} else
		kunmap(dentry_page);
...
struct f2fs_dir_entry *f2fs_find_entry(struct inode *dir, struct qstr *child,
		struct page **res_page, unsigned long *dentry_blk,
		unsigned int flags)
{
	...
	*res_page = NULL;
	...
	for (level = 0; level < max_depth; level++) {
		de = find_in_level(dir, level, &fname, res_page, dentry_blk, flags);
		if (de)
			break;
	}
out:
...
ino_t f2fs_inode_by_name(struct inode *dir, struct qstr *qstr)
{
	...
	de = f2fs_find_entry(dir, qstr, &page, NULL, CASE_SENSITIVE);
	if (de) {
